-- Produtos sem estoque permanecem visíveis na loja (não são mais marcados inativos)
-- O admin controla is_active manualmente; estoque zerado não altera visibilidade.

COMMENT ON COLUMN public.products.stock_quantity IS 'Quantidade em estoque. NULL = ilimitado. Produto permanece visível na loja mesmo com 0.';

-- Reativar produtos que estavam inativos apenas por estoque zerado
UPDATE public.products
SET is_active = true
WHERE stock_quantity = 0 AND is_active = false;

-- create_store_order: apenas diminui stock, NÃO altera is_active
...
BEGIN
  ...
  -- Inserir order_items e diminuir stock (produto permanece ativo mesmo com stock=0)
  FOR v_item IN
