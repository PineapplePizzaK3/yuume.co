...
export default function Login() {
  ...
  const defaultDash = localizedPath('appDashboard', locale)
  const from = location.state?.from?.pathname || defaultDash

  if (isAuthenticated) {
    navigate(
      needsSocialOnboarding ? localizedPath('appCompleteSocial', locale) : from,
      { replace: true },
    )
    return null
  }
    ...
    <>
      ...
      <section className="flex min-h-[calc(100vh-8rem)] items-center justify-center px-4 pt-24 pb-16">
        <div className="w-full max-w-sm rounded-lg border border-earth-200 bg-earth-100 p-6 shadow-sm">
          <h1 className="text-xl font-bold text-earth-900">{t('auth.loginTitle')}</h1>
          ...
          <p className="mt-6 text-center text-sm text-earth-600">
            {t('auth.noAccount')}{' '}
            <LocalizedLink toRoute="register" className="font-medium text-earth-900 hover:underline">
              {t('auth.signUpLink')}
            </LocalizedLink>
          </p>
        </div>
