-- Estoque só é debitado quando o pedido for PAGO, não ao criar o pedido nem ao adicionar ao carrinho.
-- create_store_order: remove decremento de stock (mantém apenas validação)
-- Trigger: ao marcar pedido como paid/products_paid, valida e debita estoque

-- 1. create_store_order: NÃO decrementa stock ao criar pedido
...
BEGIN
  ...
  -- Inserir order_items (SEM debitar estoque - será debitado quando o pedido for pago)
  FOR v_item IN
...
-- 2. Trigger: ao marcar pedido loja como paid/products_paid, validar estoque e debitar
...
DECLARE
  v_item RECORD;
BEGIN
  -- Só processar pedidos da loja que passam de awaiting_payment para paid/products_paid
...
DROP TRIGGER IF EXISTS trg_decrement_stock_on_order_paid ON public.orders;
CREATE TRIGGER trg_decrement_stock_on_order_paid
  BEFORE UPDATE OF status ON public.orders
  FOR EACH ROW
  EXECUTE FUNCTION public.decrement_stock_on_order_paid();
