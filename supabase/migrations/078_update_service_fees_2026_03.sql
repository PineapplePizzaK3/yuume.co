-- Atualiza taxas:
-- 1) Grupo de Compras / Personal Shopping: ¥250 por item.
-- 2) Redirecionamento Assistido: 15% + taxa por item igual ao Redirecionamento Padrão (aplicado no frontend).
-- No backend, este ajuste impacta create_store_order para itens de grupo.
...
BEGIN
  ...
  IF v_grupo_qty > 0 THEN
    v_grupo_fee := (v_grupo_sub * 0.20) + (250::numeric * v_fx * v_grupo_qty);
  END IF;
...
SET description = '25% do valor da compra + ¥250 por item + frete'
