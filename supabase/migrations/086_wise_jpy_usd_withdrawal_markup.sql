-- Markup JPY→USD para cobrir custo de retirada/conversão (ex.: Wise USD→JPY).
-- Padrão 0,73% (ordem de grandeza típica; ajuste em system_settings ou env no Node).

INSERT INTO public.system_settings (key, value) VALUES
  ('wise_usd_jpy_withdrawal_markup_percent', jsonb_build_object('amount', 0.73))
...
BEGIN
  ...
  v_usd := v_base_jpy * v_jpy_usd_eff;
...
COMMENT ON COLUMN public.products.price_usd IS 'Preço USD = price_jpy × fx_jpy_usd × (1 + wise markup %)';
...
SET
  price_usd = ROUND(
    (ROUND(COALESCE(p.price_jpy, p.price, 0)::numeric, 2) * s.jpy_usd_eff)::numeric,
    4
  ),
  price_brl = ROUND(
    (ROUND(COALESCE(p.price_jpy, p.price, 0)::numeric, 2) * s.jpy_usd_eff * s.usd_brl)::numeric,
    2
  )
FROM (
  SELECT
